    ...
    return (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-dark-card p-6 rounded-lg w-full max-w-4xl h-[90vh] flex flex-col shadow-2xl" onClick={(e) => e.stopPropagation()}>
                <h2 className="text-xl font-bold mb-4">Times Registrados</h2>
                ...
                <div className="flex flex-col md:flex-row gap-6 flex-grow min-h-0">
                    ...
                    <div className="md:w-1/3 flex flex-col border-r border-dark-surface pr-4">
                        ...
                        <div className="flex gap-2 pt-4 mt-auto">
                            <button onClick={handleCreateTeam} className="btn-primary w-full bg-brand-green hover:bg-green-700">Novo Time</button>
                        ...
                        {selectedTeam ? (
                            <>
                                ...
                                <form onSubmit={handleAddPlayer} className="space-y-3 mt-4 pt-4 border-t border-dark-surface">
                                    <h4 className="text-sm font-semibold">Adicionar Jogador</h4>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                        <input type="text" value={newPlayerName} onChange={(e) => setNewPlayerName(e.target.value)} placeholder="Nome do Jogador" className="input-field w-full" required />
                                        <select value={newPlayerPosition} onChange={(e) => setNewPlayerPosition(e.target.value as PlayerPosition)} className="input-field w-full">
                                            <option value={PlayerPosition.Goalkeeper}>Goleiro</option>
                                            <option value={PlayerPosition.Defender}>Defensor</option>
                                            <option value={PlayerPosition.Midfielder}>Meio-campista</option>
                                            <option value={PlayerPosition.Forward}>Atacante</option>
                                        </select>
                                    </div>
                                    <div className="flex gap-2">
                                        <input type="number" value={newPlayerNumber} onChange={(e) => setNewPlayerNumber(e.target.value)} placeholder="Nº" className="input-field flex-1" required />
                                        <button type="submit" className="btn-primary flex-1 bg-brand-blue hover:bg-blue-700 flex items-center justify-center gap-2"><PlusIcon /> Adicionar</button>
                                    </div>
                                </form>
                            </>
                        ) : (
                            <div className="flex items-center justify-center h-full text-dark-text-secondary">
                                <p>Selecione um time para editar ou crie um novo.</p>
                            </div>
                        )}
                    </div>
                ...
                <div className="flex justify-end gap-3 pt-4 mt-auto border-t border-dark-surface">
                    <button onClick={onClose} className="px-4 py-2 rounded-md bg-dark-surface hover:bg-gray-700 text-dark-text transition-colors">Cancelar</button>
                    <button onClick={handleSaveChanges} className="px-4 py-2 rounded-md bg-brand-green hover:bg-green-700 text-white transition-colors flex items-center gap-2">
                        <SaveIcon className="h-5 w-5" /> Salvar e Fechar
                    </button>
                </div>
            </div>
             <style>{`
